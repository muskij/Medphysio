...
// ---------- Settings (bank account details, etc.) ----------
...
export function getBankDetails() {
  return {
    ...
    note: getSetting("bank_note"),
  };
}

export function setBankDetails({ bankName, accountName, accountNumber, note }) {
  setSetting("bank_name", bankName || "");
  setSetting("bank_account_name", accountName || "");
  setSetting("bank_account_number", accountNumber || "");
  setSetting("bank_note", note || "");
}

// ---------- Bank-transfer payment submissions ----------
export const listPaymentSubmissions = (status) => {
  const sql = `SELECT ps.*, u.name AS user_name, u.email AS user_email
...
export const getPaymentSubmissionById = (id) =>
  db
    .prepare(
      `SELECT ps.*, u.name AS user_name, u.email AS user_email
       FROM payment_submissions ps JOIN users u ON u.id = ps.user_id WHERE ps.id = ?`
    )
    .get(id);

export const getLatestSubmissionForUser = (userId) =>
  db
    .prepare("SELECT * FROM payment_submissions WHERE user_id = ? ORDER BY created_at DESC LIMIT 1")
    .get(userId);
