...
export const getLatestTransferRequest = (userId) =>
  db.prepare("SELECT * FROM transfer_requests WHERE user_id = ? ORDER BY created_at DESC LIMIT 1").get(userId);

export const listTransferRequests = ({ status } = {}) => {
  const sql = `
    SELECT tr.*, u.name AS student_name, u.email AS student_email
    FROM transfer_requests tr JOIN users u ON tr.user_id = u.id
    ${status ? "WHERE tr.status = ?" : ""}
    ORDER BY tr.created_at DESC`;
  return status ? db.prepare(sql).all(status) : db.prepare(sql).all();
};

export const getTransferRequestById = (id) => db.prepare("SELECT * FROM transfer_requests WHERE id = ?").get(id);
